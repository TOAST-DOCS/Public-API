...
Public API는 서비스 구성에 지정된 인증 방식으로 들어오는 요청을 검증한 뒤 API 백엔드로 전달합니다. 이 문서에서는 NHN Cloud에서 제공하는 Public API의 인증 방식을 설명합니다.
...
NHN Cloud의 Public API는 서비스마다 서로 다른 인증 방식을 지원합니다. [서비스별 인증 방식](link 추가)에서 지원하는 인증 방식을 확인한 뒤 올바른 인증 방식을 사용해 인증하세요.
