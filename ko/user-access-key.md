# User Access Key
**Public API > 인증 방식 > User Access Key**
...
2. **+ User Access Key 생성**을 클릭합니다.
3. **User Access Key 생성** 모달 창에서 **토큰 유효 시간**을 설정한 뒤 **생성**을 클릭합니다.
4. **User Access Key 발급 완료** 모달 창에서 **Secret Access Key**를 복사한 뒤 **확인**을 클릭합니다.

!!! danger "주의"
    * Secret Access Key 생성 창을 닫은 뒤에는 다시 확인할 수 없습니다. Secret Access Key를 잊어버릴 경우 재생성해야 하므로 반드시 복사한 뒤 별도로 관리하세요.
...
## API 호출하기
User Access Key는 HTTP 헤더 형식을 따릅니다. API 호출 시 아래와 같이 요청 헤더에 키를 포함하여 전달합니다.
